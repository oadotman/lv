...
import { mockCalls, metricsData, mockUser } from "@/lib/mockData";
...
export default function Dashboard() {
  ...
  const recentCalls = useMemo(() => mockCalls.slice(0, 5), []);
  ...
  const dollarsSaved = metricsData.hoursSavedThisMonth * metricsData.hourlyRate;
  const percentageIncrease = Math.round(
    ((metricsData.hoursSavedThisMonth - metricsData.hoursSavedLastMonth) /
    metricsData.hoursSavedLastMonth) * 100
  );

  // Alert logic
  const totalCallsProcessed = metricsData.callsThisMonth;
  const hasNoRecentActivity = totalCallsProcessed === 0; // In production, check last 7 days
  const minutesUsagePercent = (metricsData.minutesUsed / metricsData.minutesTotal) * 100;
  const approachingLimit = minutesUsagePercent >= 80;

  // Milestone logic (check for 10, 50, 100 calls)
  const milestones = [10, 50, 100];
  const currentMilestone = milestones.find(m => totalCallsProcessed === m);

  // Auto-dismiss milestone alert after 5 seconds
  useEffect(() => {
    if (currentMilestone && !dismissedAlerts.includes(`milestone-${currentMilestone}`)) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50/30">
      ...
      <div className="p-4 lg:p-8 space-y-8 animate-in fade-in duration-200">
        ...
        <div className="space-y-4">
          {/* No Activity Alert */}
          {hasNoRecentActivity && !dismissedAlerts.includes('no-activity') && (
              ...
              <div className="flex items-start gap-4 pr-8">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-blue-700 mb-3">
                    Need help getting started? We're here to assist you with onboarding and setup.
                  </p>
                  <Link href="/help">
          ...
          {approachingLimit && !dismissedAlerts.includes('approaching-limit') && (
              ...
              <div className="flex items-start gap-4 pr-8">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-orange-700 mb-3">
                    You've used {metricsData.minutesUsed} of {metricsData.minutesTotal} minutes this month ({Math.round(minutesUsagePercent)}%). Consider upgrading to avoid interruptions.
                  </p>
                  <Link href="/settings">
          ...
          {currentMilestone && showMilestone && !dismissedAlerts.includes(`milestone-${currentMilestone}`) && (
              ...
              <div className="flex items-start gap-4 pr-8">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-emerald-700">
                    You've processed {currentMilestone} calls and saved {metricsData.hoursSavedThisMonth} hours. Keep up the great work!
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-purple-600 to-purple-700 p-6 shadow-2xl shadow-purple-500/30 animate-in fade-in duration-300">
            ...
            <div className="relative flex flex-col items-start justify-between gap-4 h-full">
              ...
              <div className="flex items-start gap-3">
                ...
                {/* Metrics */}
                <div>
                  <p className="text-white/80 text-xs font-medium mb-2">Time Saved This Month</p>
                  <div className="flex items-baseline gap-2 mb-1">
                    <h2 className="text-3xl font-bold text-white">{metricsData.hoursSavedThisMonth}</h2>
                    <span className="text-lg font-semibold text-white/90">hours</span>
                    <span className="text-xl font-bold text-white mx-1">=</span>
                    <span className="text-3xl font-bold text-white">${dollarsSaved.toLocaleString()}</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <TrendingUp className="w-3.5 h-3.5 text-emerald-300" />
                    <span className="text-white/90 text-xs font-medium">
                      +{percentageIncrease}% more than last month
                    </span>
                  </div>
                </div>
              </div>
              ...
              {/* Button */}
              <Button
                className="bg-white/20 hover:bg-white/30 text-white border-0 backdrop-blur-sm rounded-xl font-semibold px-5 py-2.5 text-sm transition-all duration-200 hover:scale-105 self-start"
              >
                View Details
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="group relative">
            <Card className="relative border border-blue-100 shadow-lg hover:shadow-xl transition-all duration-300 bg-gradient-to-br from-blue-50 to-cyan-50 overflow-hidden group-hover:-translate-y-1">
              ...
              <CardContent>
                <div className="text-4xl font-bold text-slate-900 mb-2">{metricsData.callsThisMonth}</div>
                <div className="flex items-center gap-2 text-sm font-medium">
                  <div className="flex items-center gap-1 px-2.5 py-1 bg-emerald-100 text-emerald-700 rounded-lg">
                    <TrendingUp className="w-4 h-4" />
                    <span>{metricsData.callsTrend}</span>
                  </div>
          ...
          <div className="group relative">
            <Card className="relative border border-emerald-100 shadow-lg hover:shadow-xl transition-all duration-300 bg-gradient-to-br from-emerald-50 to-green-50 overflow-hidden group-hover:-translate-y-1">
              ...
              <CardContent>
                <div className="text-4xl font-bold text-slate-900 mb-2">{metricsData.timeSaved}</div>
            ...
            <Card className="relative border border-orange-100 shadow-lg hover:shadow-xl transition-all duration-300 bg-gradient-to-br from-orange-50 to-amber-50 overflow-hidden group-hover:-translate-y-1">
              ...
              <CardContent>
                <div className="text-4xl font-bold text-slate-900 mb-3">
                  {metricsData.minutesUsed}
                  <span className="text-xl font-normal text-slate-500">/{metricsData.minutesTotal}</span>
                </div>
                <div className="relative w-full h-3 bg-orange-200 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${(metricsData.minutesUsed / metricsData.minutesTotal) * 100}%`,
                    }}
                  />
                </div>
                <p className="text-sm text-slate-600 font-medium mt-2">
                  {metricsData.minutesTotal - metricsData.minutesUsed} minutes remaining this month
                </p>
              </CardContent>
            </Card>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          <button className="group relative h-auto p-6 bg-gradient-to-br from-violet-600 to-purple-600 hover:from-violet-700 hover:to-purple-700 text-white rounded-2xl shadow-xl shadow-violet-500/30 hover:shadow-2xl hover:shadow-violet-500/50 transition-all duration-300 hover:-translate-y-1 border-0 overflow-hidden">
            <div className="absolute inset-0 bg-gradient-to-r from-white/0 via-white/20 to-white/0 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000" />
            <div className="relative flex flex-col items-center text-center gap-3">
              <div className="w-14 h-14 bg-white/20 backdrop-blur rounded-2xl flex items-center justify-center group-hover:scale-110 group-hover:rotate-6 transition-all duration-300">
                <Zap className="w-7 h-7" />
              </div>
              <div>
                <span className="font-bold text-lg block">Upload Recording</span>
                <span className="text-sm opacity-90 block mt-1">Process a new call</span>
              </div>
            </div>
          </button>

          <Link href="/templates" className="block">
            <button className="group w-full h-full p-6 bg-white hover:bg-slate-50 rounded-2xl border-2 border-slate-200 hover:border-violet-300 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
              <div className="flex flex-col items-center text-center gap-3">
                <div className="w-14 h-14 bg-gradient-to-br from-slate-100 to-slate-200 group-hover:from-violet-100 group-hover:to-purple-100 rounded-2xl flex items-center justify-center group-hover:scale-110 group-hover:rotate-6 transition-all duration-300">
                  <FileText className="w-7 h-7 text-slate-700 group-hover:text-violet-600" />
                </div>
                <div>
                  <span className="font-bold text-lg text-slate-900 block">Manage Templates</span>
                  <span className="text-sm text-slate-600 block mt-1">Customize CRM fields</span>
                </div>
              </div>
            </button>
          </Link>
          ...
          <button className="group w-full h-full p-6 bg-white hover:bg-slate-50 rounded-2xl border-2 border-slate-200 hover:border-violet-300 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div className="flex flex-col items-center text-center gap-3">
              <div className="w-14 h-14 bg-gradient-to-br from-slate-100 to-slate-200 group-hover:from-violet-100 group-hover:to-purple-100 rounded-2xl flex items-center justify-center group-hover:scale-110 group-hover:rotate-6 transition-all duration-300">
                <Download className="w-7 h-7 text-slate-700 group-hover:text-violet-600" />
              </div>
              <div>
                <span className="font-bold text-lg text-slate-900 block">Export Data</span>
                <span className="text-sm text-slate-600 block mt-1">Download all calls</span>
              </div>
            </div>
          </button>
        </div>
        ...
        <Card className="border-0 shadow-2xl rounded-3xl overflow-hidden bg-white">
          <CardHeader className="bg-gradient-to-r from-slate-50 via-purple-50/30 to-slate-50 border-b border-slate-100">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <CardTitle className="text-2xl font-bold text-slate-900 tracking-tight mb-1">Recent Calls</CardTitle>
                <p className="text-sm text-slate-600 font-medium">Your latest processed recordings</p>
              </div>
              <Link href="/calls">
          ...
          <CardContent className="p-0">
            {recentCalls.length === 0 ? (
              <div className="text-center py-20 px-6">
                ...
                <h3 className="text-2xl font-bold mb-3 text-slate-900">No calls processed yet</h3>
                <p className="text-slate-600 mb-8 max-w-md mx-auto text-lg">
                  Upload your first call recording to get AI-powered insights and CRM-ready data
                </p>
                      ...
                      >
                        <td className="px-6 py-5">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-semibold text-slate-900 group-hover:text-violet-600 transition-colors">{call.customerName}</p>
                              <p className="text-sm text-slate-500">with {call.salesRep}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-5">
                        ...
                        <td className="px-6 py-5 hidden lg:table-cell">
                          {call.sentiment && call.sentimentScore !== undefined && (
                            <TooltipProvider>
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <div
                                    className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg font-medium text-sm border cursor-default ${
                                      getSentimentConfig(call.sentiment)?.bgColor
                                    } ${getSentimentConfig(call.sentiment)?.textColor} ${
                                      getSentimentConfig(call.sentiment)?.borderColor
                                    }`}
                                  >
                                    <span className="text-base">
                                      {getSentimentConfig(call.sentiment)?.emoji}
                                    </span>
                                    <span>{getSentimentConfig(call.sentiment)?.label}</span>
                                  </div>
                                </TooltipTrigger>
                                <TooltipContent className="bg-slate-900 text-white px-3 py-2 rounded-lg">
                                  <p className="font-medium">Sentiment Score: {call.sentimentScore}/100</p>
                                </TooltipContent>
                              </Tooltip>
                            </TooltipProvider>
                          )}
